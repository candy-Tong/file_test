...
#define MAX_USER 8
int getCollegeUser(int match_id, int college_id);
...
int getSingleGrade(JoinMsg joinMsg);
...
void selectAllJoinMsg_College(int college_id) {
    ...
    for (int j = 0; j < match_num; ++j) {
        ...
        if (match.id == 0)
            continue;
        printf("\n\t%d 人参加 %s\n", getCollegeUser(match.id,college_id), match.matchName);
        printf("\t姓名\t序号\n");
        ...
        if (getCollegeUser(j,college_id)>MAX_USER) {
...
void showAllMatch(int college_id) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (getCollegeUser(match[i].id,college_id)>MAX_USER)
...
void selectOneJoinMsg_College(int college_id, int match_id) {
    ...
    if (getCollegeUser(match_id,college_id)>MAX_USER)
        printf(LIGHT_RED"\n总参加人数为 %d 我院 \t%d 人参加 %s\n"NONE,msg_n, getCollegeUser(match_id,college_id), match.matchName);
    else
        printf("\n\t%d 人参加 %s\n", getCollegeUser(match_id,college_id), match.matchName);
    for (int i = 0; i < msg_n; ++i) {
        user = selectUserById(joinMsg[i].user_id);
        if (user.college_id == college_id) {
            printf("\t%s\t序号:%d\n", user.name, joinMsg[i].id);
...
void selectCollegeGrade(int college_id) {
    ...
    for (int i = 0; i < match_n; ++i) {
        ...
        for (int j = 0; j < msg_n; ++j) {
            grade = grade + getSingleGrade(joinMsg[j]);
        }
